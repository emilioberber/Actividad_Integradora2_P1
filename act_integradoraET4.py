    ...
    def printMST(self, parent, key, channels, points_2d, ax, filename):
        ...
        for i in range(1, len(parent)):
            if parent[i] is not None and not np.isinf(key[i]):
                channel_origin = channels[parent[i]]
                channel_destination = channels[i]
                print(f"{channel_origin} - {channel_destination}", "\t|\t", f"{key[i]:.2f}")
                total_weight += key[i]
                connected_nodes.add(parent[i])
        ...
        # Imprimir nodos conectados
        print("\nConnected Nodes:", connected_nodes)

        # Calcular y dibujar el casco convexo
        connected_points = np.array([points_2d[i] for i in connected_nodes])
        hull = ConvexHull(connected_points)
        for simplex in hull.simplices:
            ax.plot(connected_points[simplex, 0], connected_points[simplex, 1], 'b--', alpha=0.7)



    # Método para imprimir nodos no conectados
    def printDisconnectedNodes(self, channels):
        disconnected_nodes = set(channels) - set(self.mst_edges)
        disconnected_nodes_names = [channels[i] for i in disconnected_nodes]
        print("\nDisconnected Nodes:", disconnected_nodes_names)
